...
function PointageDetailModal({
  ...
  async function markAsInvoiced() {
    ...
    try {
      await updateBillingStatus({ entryId, billingStatus: "facture" });
    } finally {
    ...
    <Modal open onClose={onClose} title="Détail du pointage" className="sm:h-auto sm:max-w-2xl">
      <div className="space-y-5">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
          ...
          {entry.billingStatus !== "facture" ? (
            <Button onClick={markAsInvoiced} disabled={savingStatus}>
              <BadgeCheck className="h-4 w-4" />
              {savingStatus ? "Mise à jour…" : "Mettre en Facturé"}
            </Button>
          ) : null}
        </div>
      </div>
